...
enum // the fourth byte of IS_SMALL packets is one of these
{
	...
	SMALL_SSP,		//  1 - instruction		: start sending posiztions
...
struct OutExtra
{
	float	elaTime;//time elapsed in seconds (since sending these packets started, not the race!)
	float 	lapTime;//	laptime in seconds
	float	PosFromStart;//position on track in metres from the start/finish line
	float	RacePro;//race progress, this - laps = lap progress
	float	pos_Y;	//world-Y position in metres
	float	pos_Z;//world-Z position in metres
	float	pos_X;//world-X position in metres
	float	velocity;//actual velocity in m/s
	float	vel_Y;//world-Y velocity in m/s
	float	vel_Z;//world-Z velocity in m/s
	float	vel_X;//world-X velocity in m/s
	float	Heading_1; //(relative to road direction?)
	float	Roll;
	float	Heading_2;//(relative to world?)

	float	Heading_3; //(relative to world?)
	float	Pitch;
	float	Heading_4; //(relative to road direction?)

	float	fl_sus;//suspension travel front left;
	float	fr_sus;//suspension travel front right;
	float	rl_sus;//suspension travel rear left;
	float	rr_sus;//suspension travel rear right;

	float	fl_sh;//shake travel front left;
	float	fr_sh;//shake travel front right;
	float	rl_sh;//shake travel rear left;
	float	rr_sh;//shake travel rear right;

	float	vel_rl;//velocity rear left wheel in m/s
	float	vel_rr;//velocity rear right wheel in m/s
	float	vel_fl;//velocity front left wheel in m/s
	float	vel_fr;//velocity front right wheel in m/s
	float	throttle;// throttle 0 to 1
	float	steering;// steering in quarter turns (90бу), -1.0 = 90бу to the left, 1.0 = 90бу to the right
	float	brakes;//brakes 0 to 1
	float	clutch;//clutch
	float	gear;//gear 0 Neutral, 9 Reverse, forwards speeds counting from 1
	float	latAcc;//lateral acceleration in g
	float	longAcc;//longitudinal acceleration in g
	float	lap;//lap counting from 0 (first lap = 0)
	float	rpm;//Revolutions per 6 seconds, multiply by 10.0 to get RPM
};
